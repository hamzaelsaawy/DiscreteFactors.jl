...
@testset "Factors Join" begin

@testset "main" begin
    ϕ1 = Factor(:X =>3, :C =>2, :Y =>3, :A=>2, :Z=>3)
...
@testset "2" begin
    ...
    ϕ1 = Factor(:X => 3, :Y =>3, :Z=>3)
    ϕ2 = Factor(:A=>3, :X=>31, :Y=>2)
...
@testset "singleton" begin
    ...
    @test_approx_eq ϕ3.potential ϕ1.potential * 0.1
end

@testset "both singleton" begin
    ϕ1 = Factor(3)
    ϕ2 = Factor(-2)
    ϕ3 = join(atan2, ϕ2, ϕ1)
    @test_approx_eq first(ϕ3.potential) atan2(-2, 3)
end

@testset "none common" begin
    ϕ1 = Factor(:X=>2, :Y=>3)
    ϕ2 = Factor(:A=>3, :B=>2)
    rand!(ϕ1)
    rand!(ϕ2)
    ...
    p1 = ϕ1.potential
    p2 = ϕ2.potential
    p3 = Factors.duplicate(p1, 3, 2)
    p3 .*= reshape(p2, (1, 1, 3, 2))

    @test_approx_eq ϕ3.potential p3
end
